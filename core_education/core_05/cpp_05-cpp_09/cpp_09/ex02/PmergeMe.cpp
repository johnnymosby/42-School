...
//deque
int binary_search(std::deque <int> const &vec, int target) {
	int low = 0;
	int high = vec.size() - 1;
...
void sort(std::deque<int> &vec_orig) {
	if (vec_orig.size() <= 1)
		return ;

	std::deque<int>::iterator begin = vec_orig.begin();
	std::deque<int>::iterator end = vec_orig.end();
	std::deque<int>::iterator middle = vec_orig.size() / 2 + begin;
	std::deque<int> left(begin, middle), right;
	int leftover = 0;
	if (vec_orig.size() % 2 != 0) {
		end--;
		leftover = *(end);
	}
	right = std::deque<int>(middle, end);
	for (u_long i = 0; i < (vec_orig.size() / 2); i++) {
		if (left[i] > right[i])
			std::swap(left[i], right[i]);
	}
	std::deque<int> old_right = right;
	sort(right.begin(), right.end());
	std::deque<int> old_left = left;
	for (u_long i = 0; i < right.size(); i++) {
		std::deque<int>::iterator it = std::find(old_right.begin(), old_right.end(), right[i]);
		if (it != old_right.end()) {
			int index = std::distance(old_right.begin(), it);
			left[i] = old_left[index];
		}
	}
	std::deque<int> result = right;
	if (leftover != 0) {
		left.push_back(leftover);
	}
	result.insert(result.begin(), left[0]);
